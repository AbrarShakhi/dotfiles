...
git() {
  ...
  # Ensure GITHUB_KEY is set and exists
  if [ -z "$GITHUB_KEY" ] || [ ! -f "$GITHUB_KEY" ]; then
    if command -v fzf > /dev/null 2>&1; then
      input_key=$(find ~/.ssh -type f ! -name '*.pub' 2>/dev/null | fzf \
  ...
  if ! ssh-add -l | grep -q "$(ssh-keygen -lf "$GITHUB_KEY" | awk '{print $2}')" 2>/dev/null; then
